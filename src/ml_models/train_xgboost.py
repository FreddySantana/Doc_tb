"""
Módulo para treinamento de modelo XGBoost
...
Implementa treinamento de XGBoost conforme descrito na Seção 4.3 da tese.
XGBoost é um dos algoritmos de boosting mais eficazes para dados tabulares.

Referência: Chen, T., & Guestrin, C. (2016). XGBoost: A scalable tree boosting system.
"""
...
class XGBoostTrainer:
    """
    Treinador de modelo XGBoost.
    
    XGBoost (Extreme Gradient Boosting) implementa gradient boosting
    com regularização L1 e L2 para prevenir overfitting.
    ...
    def __init__(self, config: Dict[str, Any] = None, random_state: int = None):
        ...
        # Hiperparâmetros padrão otimizados para dados desbalanceados
        self.params = {
            'max_depth': 6,
            'learning_rate': 0.1,
            'n_estimators': 100,
            ...
            'n_jobs': -1,
            'scale_pos_weight': 1,  # Será ajustado automaticamente
            'subsample': 0.8,
        ...
        self.output_dir = Path('results/ml_models/xgboost')
        self.output_dir.mkdir(parents=True, exist_ok=True)
        
        logger.info('XGBoostTrainer inicializado')
    
    def _calculate_scale_pos_weight(self, y_train: pd.Series) -> float:
        """
        Calcula o peso para balancear classes.
        ...
        logger.info(f'Classe negativa: {neg_count}, Classe positiva: {pos_count}')
        logger.info(f'scale_pos_weight calculado: {scale_pos_weight:.2f}')
    ...
    def train(
        ...
        # Ajustar scale_pos_weight automaticamente
        ...
        if X_val is not None and y_val is not None:
            ...
            self.training_history['train_loss'] = self.model.evals_result()['validation_0']['logloss']
            self.training_history['val_loss'] = self.model.evals_result()['validation_1']['logloss']
        else:
    ...
    def evaluate(
        ...
        """
        logger.info('='*80)
        logger.info('AVALIANDO MODELO XGBOOST')
        logger.info('='*80)
    ...
    def save(self, filename: str = 'xgboost_model.pkl') -> None:
        """
...
def main():
    ...
    logger.info('Iniciando treinamento de XGBoost')
    ...
    print('\n' + '='*80)
    print('✅ TREINAMENTO DE XGBOOST CONCLUÍDO COM SUCESSO!')
    print('='*80)
    print(f'\n📊 Métricas Finais:')
