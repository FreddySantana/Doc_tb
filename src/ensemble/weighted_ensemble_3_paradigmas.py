...
class WeightedEnsemble3Paradigms:
    ...
    def calculate_uncertainty(
        ...
        """
        Calcula a quantificação de incerteza (Equações 82-84 da tese).
        ...
        # Criar instância de quantificação de incerteza
    ...
    def evaluate(
        ...
        # Calcular incerteza
        u_total, u_mc, u_ens = self.calculate_uncertainty(
            ml_proba, drl_proba, nlp_proba, mc_samples
        )
        ...
        metrics = {
            ...
            'uncertainty_total_mean': np.mean(u_total),
            'uncertainty_total_std': np.std(u_total),
            'uncertainty_mc_mean': np.mean(u_mc),
            'uncertainty_ens_mean': np.mean(u_ens)
        }
